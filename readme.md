...
After running `terraform apply --var-file=secrets.tfvars` inside the backend directory inside the `development` directory and intializing the backend successfully, put the backend blob details inside the `backend` block inside the `providers.tf` located inside the `development` directory.

Run the `terraform init` command inside the `development` directory to initialize the needed modules.

Run the `terraform plan --var-file=secrets.tfvars` command to create a plan of the resources that should be created and then run `terraform apply `
